...
task 'assets:precompile:before' do
  ...
  $node_compress = false
  if !ENV['SKIP_NODE_UGLIFY']
    $node_compress = `which terser`.present? || `which uglifyjs`.present?
  end
  ...
  if ENV['EMBER_CLI_PROD_ASSETS']
    ...
    Rails.configuration.assets.precompile -= %w(application.js admin.js ember_jquery.js pretty-text-bundle.js start-discourse.js markdown-it-bundle.js)
